...
import { spacesData, projectsData, taskData } from "./data";
...
async function getERPList() {
  try {
    const promises = spacesData.map(async (erp) => {
      const projectsDataFiltered = projectsData.filter((project) => project.space === erp.name);
      const sub = projectsDataFiltered.map((project) => {
        return {
